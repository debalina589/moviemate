...
const MyBookings = () => {
  ...
  return !isLoading ? (
    <div className='relative px-6 md:px-16 lg:px-40 pt-32 md:pt-40 min-h-[80vh]'>
      ...
      {bookings.length === 0 ? (
        <p className='text-gray-300 mt-4'>You have no Bookings yet.</p>
      ) : 
      bookings.map((item, index) => (
        <div key={index} className='flex flex-col md:flex-row justify-between bg-blue-500/80 border-blue/20 rounded-lg mt-4 p-2 max-w-3xl'>
          ...
          <div className='flex flex-col md:items-end md:text-right justify-between p-4'>
            <div className='flex items-center gap-4'>
              ...
              {!item.isPaid && <Link to={item.paymentLink} className='bg-pink-400 px-4 py-1.5 mb-3 text-sm rounded-full font-medium cursor-pointer'>Pay Now</Link>}
            </div>
